...
namespace w9_efcore_intro.Services
{
    public class GameEngine
    {
        ...
        private void GiveItemtoPlayer(Character player)
        {
            ...
            _context.Items.Add(bow);
            _context.Items.Add(club);

            // the lower number is inclusive and the upper number is exclusive
            // so this will generate a random number between 1 and 3
            int rNum = new Random().Next(1, 4);
            var itemToGive = rNum switch
            {
                1 => sword,
                2 => bow,
                3 => club,
